...
simulated window spent is scaled up to 30 days, as if the same traffic repeated all month.
"""
...
def monthly_cost(design: Design, nodes: dict[str, Node], duration_s: float) -> Cost:
    """The monthly bill, one line per node that costs anything. `nodes` are the simulated nodes, read
    after the run for what hosted LLM calls spent."""
    lines = []
    for node in design.nodes:
        match node:
            ...
            case LlmNode(params=HostedLlmParams()):
                ...
                usd = spent * MONTH_S / duration_s
                detail = f"${spent:.4f} of LLM tokens in {duration_s:g} s, repeated for 30 days"
